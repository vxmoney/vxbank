...
public class UserUtils {
    ...
    public static UserStripeLinkResponse getStripeLoginLink(TestRestTemplate restTemplate,
                                                            int port,
                                                            String vxToken,
                                                            int expectedStatusCode) {
        HttpHeaders headers = new HttpHeaders();
        ...
        // Make the GET request to /ping/whoAmI
        ResponseEntity<UserStripeLinkResponse> responseEntity = restTemplate.exchange(
                "http://localhost:" + port + "/user/getStripeLoginLink",
                HttpMethod.GET,
                requestEntity,
                UserStripeLinkResponse.class);

        int statusCode = responseEntity.getStatusCodeValue();
        Assertions.assertEquals(expectedStatusCode, statusCode);

        UserStripeLinkResponse response = responseEntity.getBody();
        return response;
    }
}
